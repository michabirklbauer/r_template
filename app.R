...
ui <- pageWithSidebar(
    headerPanel = headerPanel("Clustering App"),
    sidebarPanel = sidebarPanel(
        selectInput("top_x", "Length or Width", choices = c("Length", "Width"), selected = "Length"),
        uiOutput("xselect"),
        selectInput("top_y", "Length or Width", choices = c("Length", "Width"), selected = "Width"),
        uiOutput("yselect")
...
server <- function(input, output, session){
    ...
    output$xselect <- renderUI({
        ...
        selection <- colnames(iris)[grepl(pattern = paste0(input$top_x, "$"), colnames(iris))]
    ...
    output$yselect <- renderUI({
        ...
        selection <- colnames(iris)[grepl(pattern = paste0(input$top_y, "$"), colnames(iris))]
